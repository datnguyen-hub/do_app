...
import 'ctv_wallet_screen/ctv_wallet_controller.dart';

class RegisterCtvScreen extends StatelessWidget {
  final Widget child;
  DataAppCustomerController dataAppCustomerController = Get.find();

  RegisterCtvScreen({Key? key, required this.child}) : super(key: key);

  RegisterCtvController registerCtvController =RegisterCtvController();
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(
      () =>
          dataAppCustomerController.infoCustomer.value.isCollaborator == true &&
                  dataAppCustomerController.infoCustomer.value.isCollaborator !=
                      null
              ? child
              : Scaffold(
                  appBar: AppBar(
                    title: Text("Điều khoản Cộng tác viên"),
                  ),
                    ...
                    child: Column(
                      children: [
                        SahaButtonFullParent(
                          ...
                          onPressed: () {
                            Get.to(()=>UpdateInfoCollaboratorScreen())!.then((value) {
                              if(value == "Success"){
                                registerCtvController.registerCtv();
                              }
                            });
                            //registerCtvController.registerCtv();
                          },
...
class RegisterCtvController extends GetxController {
  DataAppCustomerController dataAppCustomerController = Get.find();
  late CtvWalletController ctvWalletController;
  Future<void> registerCtv() async {
    ctvWalletController = Get.find();
    try {
      ...
      Future.delayed(Duration(milliseconds: 100), () async {
        ...
        ctvWalletController.getGeneralInfoPaymentCtv();
